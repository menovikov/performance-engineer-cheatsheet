...
- separate physical operations of reading/writing from same logical operations, e.g. creating a buffer and then flushing it to disk at once.
- interact with memory and other caches to simplify access to data
...
- has random and sequential I/O (input/output), difference - random operations start from different places on disk, sequential - from the same place, which makes them faster (analogous to consecutive iterations over a tree vs random access to elements).
...
- has actual processes running and occupying CPU and memory, so all affinity techniques and CPU optimizations are applicable here as well.
- IOPS don't mean much without knowing the size of data being read/written. For example, 1000 IOPS with 1MB block size is 1GB/s, but with 4KB block size is only 4MB/s.
...
1. Analyze disks
   ...
   - check disk speed and load:
     ```shell
        root@ubuntu-DE-Frankfurt-1gb:~# iostat
     ...
     ```shell
     root@ubuntu-DE-Frankfurt-1gb:~# iostat -sxz
     Linux 6.8.0-45-generic (ubuntu-DE-Frankfurt-1gb) 	10/29/24 	_x86_64_	(1 CPU)
...
3. Analyze and restructure load itself.
   ...
   - is the disk full? consider moving some data to another disk or compressing it, otherwise write operations may be slow.
...
4. Setup caching
   - cloud providers usually setup caches automatically, read here (link) how AWS supports its FXs for OpenZFS: [link](https://docs.aws.amazon.com/fsx/latest/OpenZFSGuide/performance.html). 
...
2. For best performance choose NVMe SSDs with RAID 0. It doesn't provide redundancy, but it is the fastest way to read/write data. When loosing data is critical, use RAID 1 or RAID 10.
...
2. Use `slabtop -o` to see kernel caches stats:
...
3. Use `strace -ttT -p <pid>` to look for system calls to file system (read, write, open, close, etc.) and their duration:
   ```shell
    $ strace -ttT -p <pid>
    [...]
...
4. Use `fatrace` to see what files are being accessed on the filesystem.
5. Use `filetop` to see which files are being accessed the most.
6. Use `opensnoop -T` to see which files are being opened and closed.
7. Use `cachestat -T 1` to see basic cache stats:
    ```shell
    $ cachestat -T 1
...
8. Use `ext4dist 10 1` to see read/write/open/fsync latencies distributions.
9. Use `ext4slower` to see the slowest filesystem operations.